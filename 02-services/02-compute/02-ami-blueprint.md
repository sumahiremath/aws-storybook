---
description: "An Amazon Machine Image captures the approved starting configuration used to launch repeatable EC2 instances."
tags:
  - "aws"
  - "compute"
  - "ec2"
...
# Amazon Machine Image (AMI): Bottle the Shop Setup, Not the Business

> An Amazon Machine Image captures the approved starting configuration used to launch repeatable EC2 instances.

## The Business Goal

The first Bits Boba location took weeks to prepare.
...
The original Bits Boba location is running beautifully.
...
In Bits Boba, the AMI captures the systems that every approved location should begin with.
...
For the Bits Boba, the AMI is the approved operating package.
...
In the Bits Boba, the launch template is the opening order for a particular kind of location.
...
In the Bits Boba, the golden image is the company-approved starter shop before a particular product team installs its menu application.
...
The first Bits Boba location proved the business could run.
...
**Next chapter:** *[Amazon EC2 Auto Scaling: The Fleet Manager Who Opens Bits Boba Locations Before the Line Gets Too Long](03-auto-scaling-the-fleet-manager.md)*
