---
description: "Amazon EC2 Auto Scaling automatically adjusts the number of EC2 instances so your application has the capacity it needs without keeping unnecessary servers running."
tags:
  - "aws"
  - "compute"
  - "auto-scaling"
...
# Amazon EC2 Auto Scaling: The Fleet Manager Who Opens Bits Boba Locations Before the Line Gets Too Long
...
Yesterday, one Bits Boba location was enough.
...
The fleet manager does not author the software, choose the instance type, or define the menu. It launches the configuration declared by the launch template, AMI, and bootstrap instructions.
...
| Bits Boba | AWS | Meaning |
|---|---|---|
| Fleet manager | Auto Scaling Group | Maintains the fleet |
| Bits Boba location | EC2 instance | Compute capacity |
...
The group does not intentionally set desired capacity below the minimum. Failures or capacity shortages can still leave fewer healthy instances running while Auto Scaling attempts replacement.

It tries to maintain the desired capacity.

The maximum normally bounds desired capacity, although documented features such as Capacity Rebalancing can temporarily launch above it while replacing at-risk instances.
...
**Next chapter:** *[Elastic Load Balancing: The Host Who Seats Every Customer](04-elastic-load-balancing-the-host.md)*
